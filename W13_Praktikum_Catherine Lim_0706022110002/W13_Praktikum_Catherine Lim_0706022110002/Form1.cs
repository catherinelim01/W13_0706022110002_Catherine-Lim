...
namespace W13_Praktikum_Catherine_Lim_0706022110002
{
    public partial class FormPemain : Form
    {
        ...
        {
            sqlQuery = "select p.player_id, p.player_name, p.birthdate, n.nation, t.team_name , p.team_number, p.team_id, p.nationality_id from player p, nationality n, team t where p.nationality_id = n.nationality_id and p.team_id = t.team_id;";
        ...
        public void IsiDataPemain(int Posisi)
        {
            ...
            PlayerIDSebelumnya = dataPlayer.Rows[Posisi][0].ToString();
            PlayerNameSebelumnya = dataPlayer.Rows[Posisi][1].ToString();
            BirthDateSebelumnya = dataPlayer.Rows[Posisi][2].ToString();
            NationalitySebelumnya = dataPlayer.Rows[Posisi][7].ToString();
            TeamSebelumnya = dataPlayer.Rows[Posisi][6].ToString();
            TeamNumberSebelumnya = dataPlayer.Rows[Posisi][5].ToString();
        ...
        {
            if (label_NotAvailable.Text == "Not Available")
            {
                MessageBox.Show("Team Number Not Available, please change it first");
            }
            else
            {
                if(comboBox_Team.SelectedValue.ToString() == TeamSebelumnya)
                {
                ...
                //MessageBox.Show("UPDATE player SET player_id ='" + textBox_PlayerID.Text + "', player_name ='" + textBox_PlayerName.Text + "', birthdate = date_format("+ dateTimePicker_BirthDate.Text + ", '%Y-%m-%d'), nationality_id = '" + comboBox_Nationality.SelectedValue.ToString() + "', team_id ='" + comboBox_Team.SelectedValue.ToString() + "', team_number ='" + numericUpDown_TeamNumber.Value.ToString() + "' where player_id = '" + PlayerIDSebelumnya + "' and player_name = '" + PlayerNameSebelumnya + "' and birthdate = date_format(" + BirthDateSebelumnya +", '%Y-%m-%d') and nationality_id ='" + NationalitySebelumnya + "' and team_id ='" + TeamSebelumnya + "'and team_number = '" + TeamNumberSebelumnya + "';");
                sqlQuery = "UPDATE player SET player_id ='" + textBox_PlayerID.Text + "', player_name ='" + textBox_PlayerName.Text + "', nationality_id = '" + comboBox_Nationality.SelectedValue.ToString() + "', team_id ='" + comboBox_Team.SelectedValue.ToString() + "', team_number ='" + numericUpDown_TeamNumber.Value.ToString() + "' where player_id = '" + PlayerIDSebelumnya + "' and player_name = '" + PlayerNameSebelumnya + "' and nationality_id ='" + NationalitySebelumnya + "' and team_id ='" + TeamSebelumnya + "'and team_number = '" + TeamNumberSebelumnya + "';";
